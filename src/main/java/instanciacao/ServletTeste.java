...
public class ServletTeste extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		ParticipacaoServico ps = new ParticipacaoServico();
			...
			/*
			//Tentando atualizar Artista
			Artista a1 = as.buscar(1);
			response.getWriter().append("FORA DO TRY\n");
			try {
				a1.setCache(a1.getCache().add(new BigDecimal("4.00")));
				as.Atualizar(a1);
		...
		/*
		// Tentando atualizar o nome de um artista(2)
		Artista a2 = as.buscar(2);
		try {
			a2.setNome("Peter Parker");
			as.Atualizar(a2);
			response.getWriter().append("Artista atualizado\n");
		}catch(ServicoException e) {
			response.getWriter().append(e.getMessage()+"\n");
