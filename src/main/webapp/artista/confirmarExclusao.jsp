...
Objetivo sucinto do programa: pagina para confirmação da exclusão como validação
...
<body>
	...
	<div class="container">
		...
		<div>
			...
			<ul class="list-group">
				<li class="list-group-item">Codigo: ${item.codArtista}</li>
				...
				<li class="list-group-item">Cache: <fmt:setLocale value="pt_BR" /> <fmt:formatNumber
						type="currency" value="${item.cache}"></fmt:formatNumber></li>
				<li class="list-group-item">Nascimento: <fmt:formatDate type="date" pattern="dd/MM/yyyy"
						value="${item.nascimento}" /></li>
			</ul>
		</div>
		<div>
			<a href="<%=request.getContextPath()%>/artista/excluir?cod=${item.codArtista}"
				class="btn btn-danger">Apagar</a> <a
